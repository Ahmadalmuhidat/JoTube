...
router.post('/upload', requireAuth, upload.fields([{ name: 'video', maxCount: 1 }, { name: 'thumbnail', maxCount: 1 }]), (req, res) => videoController.upload(req, res));
...
router.put('/:id', requireAuth, upload.fields([{ name: 'thumbnail', maxCount: 1 }]), (req, res) => videoController.update(req, res));
